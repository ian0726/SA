    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            <?php
              $sqll = "SELECT * FROM `orderdetail` WHERE reply IS NULL AND det_id ='" . $rowtemp['det_id'] . "'";
              if ($rss = mysqli_query($con, $sqll)) {
                while ($roww = mysqli_fetch_assoc($rss)) {
                  echo "<textarea name='feedbacktext' rows='4' cols='25'>$feedback</textarea>";
                }
              }
          ...
          <td>
            <?php
              ...
              if ($rss = mysqli_query($con, $sqll)) {
                while ($roww = mysqli_fetch_assoc($rss)) {
                  echo "<input type='submit' class='btn btn-success btn-xs btn-edit' value='確認送出'>";
                  break;
                }
              }
              $sqll = "SELECT * FROM `orderdetail` WHERE reply IS NOT NULL AND det_id ='" . $rowtemp['det_id'] . "'";
